...
export class TxStoreManager {
  ...
  constructor(
    {
      workdir = '/tmp/test/',
      inMemory = false,
      autoCompactionInterval = 0,
      recentActionAvoidRepeatDistanceBlocks = 0,
    },
    ...
    if (autoCompactionInterval !== 0) {
      this.txstore.persistence.setAutocompactionInterval(
        autoCompactionInterval
      );
    }
    this.txstore.ensureIndex({ fieldName: 'txId', unique: true });
    this.txstore.ensureIndex({ fieldName: 'nonceSigner', unique: true });

    const dbLocationStr = inMemory
      ? 'memory'
      : `${workdir}/${TXSTORE_FILENAME}`;
  ...
  async putTx(
    ...
    const existing = await this.txstore.findOneAsync({
  ...
  async getTxsInNonceRange(
    signer: PrefixedHexString,
    fromNonce: number,
    toNonce: number = Number.MAX_SAFE_INTEGER
  ): Promise<StoredTransaction[]> {
    return (
      await this.txstore.findAsync({
        $and: [
  ...
  async removeArchivedTransactions(
    upToMinedBlockNumber: number,
    upToMinedTimestamp: number
  ): Promise<unknown> {
   ...
   */
  async isActionPendingOrRecentlyMined(
    serverAction: ServerAction,
    currentBlock: number,
    recencyBlockCount: number,
    destination: Address | undefined = undefined
  ): Promise<boolean> {
    const allTransactions = await this.getAll();
    const storedMatchingTxs = allTransactions.filter(
      (it) =>
        it.serverAction === serverAction &&
        (destination == null || isSameAddress(it.to, destination))
    );
    const pendingTxs = storedMatchingTxs.filter(
      (it) => it.minedBlock?.number == null
    );
    if (pendingTxs.length !== 0) {
      this.logger.info(
        `Found ${pendingTxs.length} pending transactions that match a query: ${JSON.stringify(pendingTxs)}`
      );
      return true;
    }
    const recentlyMinedTxs = storedMatchingTxs.filter((it) => {
      ...
      return (
        ...
        currentBlock - minedBlockNumber <= recencyBlockCount
      );
    });
    if (recentlyMinedTxs.length !== 0) {
      this.logger.info(
        `Found ${recentlyMinedTxs.length} recently mined transactions that match a query: ${JSON.stringify(recentlyMinedTxs)}`
      );
      return true;
    }
    return false;

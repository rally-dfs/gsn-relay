...
export class ReputationStoreManager {
  ...
  constructor(
    { workdir = '/tmp/test/', inMemory = false },
    logger: LoggerInterface
  ) {
    this.logger = logger;
    const filename = inMemory
      ? undefined
      : `${workdir}/${REPUTATION_STORE_FILENAME}`;
  ...
  async updateLastAcceptedTimestamp(paymaster: Address): Promise<void> {
    ...
    this.logger.debug(
      `Paymaster ${paymaster} was last accepted at ${lastAcceptedRelayRequestTs}`
    );
    await this.updateEntry(paymaster, update);
  }

  async updatePaymasterReputation(
    paymaster: Address,
    change: number,
    ...
    if (eventBlockNumber <= oldChangesExpirationBlock) {
      throw new Error(
        `Invalid change expiration parameter! Passed ${oldChangesExpirationBlock}, but event was emitted at block height ${eventBlockNumber}`
      );
    }
    const existing: ReputationEntry = await this.txstore.findOneAsync({
      paymaster: paymaster.toLowerCase(),
    });
    ...
    const changes = [...existing.changes, reputationChange].filter(
      (it) => it.blockNumber > oldChangesExpirationBlock
    );
    const update: Partial<ReputationEntry> = {
      reputation,
      changes,
    ...
    this.logger.info(
      `Paymaster ${paymaster} reputation changed from ${existing.reputation} to ${reputation}. Change is ${change}`
    );
    await this.updateEntry(paymaster, update);
  }
  ...
  private async updateEntry(
    paymaster: Address,
    update: Partial<ReputationEntry>
  ): Promise<void> {
    const existing: ReputationEntry = await this.txstore.findOneAsync({
      paymaster: paymaster.toLowerCase(),
    });
    const entry = Object.assign({}, existing, update);
    await this.txstore.updateAsync(
      { paymaster: existing.paymaster },
      { $set: entry }
    );
  }

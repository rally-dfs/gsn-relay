...
export class RegistrationManager {
  ...
  async init(
    ...
    const minimumStakePerToken =
      await this.contractInteractor.getMinimumStakePerToken(
        this.config.managerStakeTokenAddress
      );
  ...
  async handlePastEvents(
    ...
    for (const eventData of decodedEvents) {
      switch (eventData.event) {
        case HubAuthorized:
          this.logger.warn(
            `Handling HubAuthorized event: ${JSON.stringify(eventData)} in block ${currentBlock.number}`
          );
        ...
        case OwnerSet:
          ...
          this.logger.warn(
            `Handling OwnerSet event: ${JSON.stringify(eventData)} in block ${currentBlock.number}`
          );
          break;
        case StakeAdded:
          transactionHashes = transactionHashes.concat(
          ...
          this.logger.warn(
            `Handling StakeAdded event: ${JSON.stringify(eventData)} in block ${currentBlock.number}`
          );
          break;
        case HubUnauthorized:
          this.logger.warn(
            `Handling HubUnauthorized event: ${JSON.stringify(eventData)} in block ${currentBlock.number}`
          );
        ...
        case StakeUnlocked:
          this.logger.warn(
            `Handling StakeUnlocked event: ${JSON.stringify(eventData)} in block ${currentBlock.number}`
          );
          transactionHashes = transactionHashes.concat(
            await this.refreshStake(
              currentBlock.number,
        ...
        case StakeWithdrawn:
          this.logger.warn(
            `Handling StakeWithdrawn event: ${JSON.stringify(eventData)} in block ${currentBlock.number}`
          );
          transactionHashes = transactionHashes.concat(
            await this.refreshStake(
              currentBlock.number,
  ...
  async refreshRegistrarRelayInfo(): Promise<void> {
    try {
      this.currentRelayInfo = await this.contractInteractor.getRelayInfo(
        this.managerAddress
      );
    } catch (error: any) {
  ...
  _isRegistrationCorrect(): boolean {
    return (
      this.currentRelayInfo != null &&
      this.currentRelayInfo.relayUrl === this.config.url
    );
  }
  ...
  async refreshStake(
    ...
    if (stakedOnHubStatus.isStaked) {
      ...
      stakedOnHubStatus.errorMessage === 'this hub is not authorized by SM'
    ) {
      const isAuthorizePending =
        await this.txStoreManager.isActionPendingOrRecentlyMined(
          ServerAction.AUTHORIZE_HUB,
          currentBlockNumber,
          this.config.recentActionAvoidRepeatDistanceBlocks
        );
      if (!isAuthorizePending) {
        const authorizeHubByManagerMethod =
          await this.contractInteractor.getAuthorizeHubByManagerMethod();
        ...
        this.logger.warn(
          `Currently configured RelayHub is not authorized! Authorizing new RelayHub: ${this.hubAddress}`
        );
  ...
  async attemptRegistration(
    ...
    this.logger.debug(
      `Relay ${this.managerAddress} registered on hub ${this.hubAddress} via registrar ${registrarAddress}. `
    );
    return transactions;
  }
  ...
  async _sendManagerEthBalanceToOwner(
    ...
    if (managerBalance.gte(txCost)) {
      ...
      this.logger.error(
        `manager balance too low: ${managerBalance.toString()}, tx cost: ${gasLimit * parseInt(gasPrice)}`
      );
    }
    return transactionHashes;
  }
  ...
  async _sendWorkersEthBalancesToOwner(
    ...
    if (workerBalance.gte(txCost)) {
      ...
      this.logger.info(
        `balance too low: ${workerBalance.toString()}, tx cost: ${gasLimit * parseInt(gasPrice)}`
      );
    }
    return transactionHashes;
  }
...
`;
  ...
  printEvents(decodedEvents: EventData[], options: PastEventOptions): void {
    ...
    this.logger.info(
      `Handling ${decodedEvents.length} events emitted since block: ${options.fromBlock?.toString()}`
    );

...
export class RelayServer extends EventEmitter {
  ...
  validateRequestTxType(req: RelayTransactionRequest): void {
    if (
      ...
      throw new Error(
        `Current network (${this.chainId}) does not support EIP-1559 transactions.`
      );
    }
  }
  ...
  validateWhitelistsAndBlacklists(relayRequest: RelayRequest): void {
    if (this._isBlacklistedPaymaster(relayRequest.relayData.paymaster)) {
      throw new Error(
        `Paymaster ${relayRequest.relayData.paymaster} is blacklisted!`
      );
    }
    ...
    if (this.config.url.length === 0) {
      if (!this._isWhitelistedPaymaster(relayRequest.relayData.paymaster)) {
        throw new Error(
          `Paymaster ${relayRequest.relayData.paymaster} is not whitelisted!`
        );
      }
      if (!this._isWhitelistedRecipient(relayRequest.request.to)) {
        throw new Error(
          `Recipient ${relayRequest.request.to} is not whitelisted!`
        );
      }
    }
  }
  ...
  async calculateAndValidatePaymasterGasAndDataLimits(
    ...
    const trustedPaymasterGasAndDataLimits =
  ...
  async validatePaymasterGasAndDataLimits(
    ...
    this.verifyTransactionCalldataGasUsed(
      relayTransactionRequest,
      relayRequestLimits.transactionCalldataGasUsed
    );
    this.verifyEffectiveAcceptanceBudget(
      relayRequestLimits.paymasterAcceptanceBudget,
      relayRequestLimits.effectiveAcceptanceBudget,
      parseInt(relayTransactionRequest.metadata.maxAcceptanceBudget),
      paymaster
    );
    this.verifyMaxPossibleGas(relayRequestLimits.maxPossibleGas);
    await this.verifyPaymasterBalance(
      relayRequestLimits.maxPossibleCharge,
      relayRequestLimits.maxPossibleGas,
      paymaster
    );
  }

  verifyTransactionCalldataGasUsed(
    ...
    if (
      toBN(transactionCalldataGasUsed).gt(
  ...
  verifyEffectiveAcceptanceBudget(
    ...
    if (effectiveAcceptanceBudget > this.config.maxAcceptanceBudget) {
      ...
      this.logger.debug(
        `Using trusted paymaster's higher than max acceptance budget. requestMaxAcceptanceBudget: ${requestMaxAcceptanceBudget} effectiveAcceptanceBudget: ${effectiveAcceptanceBudget}`
      );
    }
  }
  ...
  verifyMaxPossibleGas(maxPossibleGasFactorReserve: number): void {
    if (maxPossibleGasFactorReserve > this.maxGasLimit) {
      throw new Error(
        `maxPossibleGas (${maxPossibleGasFactorReserve}) exceeds maxGasLimit (${this.maxGasLimit})`
      );
    }
  }

  async verifyPaymasterBalance(
    maxPossibleCharge: BN,
    maxPossibleGasFactorReserve: number,
    paymaster: string
  ): Promise<void> {
    const paymasterBalance = await this.relayHubContract.balanceOf(paymaster);
    this.logger.debug(
      `paymaster balance: ${paymasterBalance.toString()}, maxCharge: ${maxPossibleCharge.toString()}`
    );
    this.logger.debug(
      `Estimated max charge of relayed tx: ${maxPossibleCharge.toString()}, GasLimit of relayed tx: ${maxPossibleGasFactorReserve}`
    );
    if (paymasterBalance.lt(maxPossibleCharge)) {
      throw new Error(
        `paymaster balance too low: ${paymasterBalance.toString()}, maxCharge: ${maxPossibleCharge.toString()}`
      );
    }
  }
...
`);
  ...
  async createRelayTransaction(
    ...
    await this.validateViewCallSucceeds(
      req,
      parseInt(req.metadata.maxAcceptanceBudget),
      maxPossibleGas
    );
    if (this.config.runPaymasterReputations) {
    ...
    const method = this.relayHubContract.contract.methods.relayCall(
      req.metadata.domainSeparatorName,
      req.metadata.maxAcceptanceBudget,
      req.relayRequest,
      req.metadata.signature,
      req.metadata.approvalData
    );
    ...
    const { signedTx, nonce } =
      await this.transactionManager.sendTransaction(details);
    const nonceGapFilled = await this.transactionManager.getNonceGapFilled(
      this.workerAddress,
      req.metadata.relayLastKnownNonce,
      nonce - 1
    );
  ...
  async _initTrustedPaymasters(paymasters: string[] = []): Promise<void> {
    ...
    for (const paymasterAddress of paymasters) {
      const paymaster =
        await this.contractInteractor._createPaymaster(paymasterAddress);
  ...
  async init(): Promise<PrefixedHexString[]> {
    ...
    const transactionHashes = await this.registrationManager.init(
      this.lastScannedBlock,
      latestBlock
    );
...
`);
  ...
  async replenishServer(
    ...
    // get balances
    let managerEthBalance =
      this.registrationManager.balanceRequired.currentValue;
    ...
    const mustReplenishWorker =
      !this.workerBalanceRequired.isSatisfied && !isReplenishPendingForWorker;
    const mustReplenishManager =
      toBN(this.config.managerMinBalance).gt(managerEthBalance) &&
      !isWithdrawalPending;
    ...
    const workerReplenishAmount = toBN(
      this.config.workerTargetBalance.toString()
    ).sub(this.workerBalanceRequired.currentValue);
    const managerReplenishAmount = toBN(
      this.config.managerTargetBalance.toString()
    ).sub(managerEthBalance);
    const canReplenishManager = managerHubBalance.gte(managerReplenishAmount);
    const cantReplenishWorkerFromBalance = managerEthBalance
      .sub(toBN(this.config.managerMinBalance))
      .lt(workerReplenishAmount);
    const canReplenishWorkerFromHubAndBalance = managerHubBalance
      .add(managerEthBalance)
      .sub(toBN(this.config.managerMinBalance))
      .gte(workerReplenishAmount);
    const mustWithdrawHubDeposit =
      (mustReplenishManager && canReplenishManager) ||
      (mustReplenishWorker &&
        cantReplenishWorkerFromBalance &&
        canReplenishWorkerFromHubAndBalance);

    if (mustWithdrawHubDeposit) {
      const transactionHash = await this._withdrawHubDeposit(
        managerHubBalance,
        currentBlockNumber,
        currentBlockHash,
        currentBlockTimestamp
    ...
    if (mustReplenishWorker) {
      ...
      if (
        workerReplenishAmount.lt(
          managerEthBalance.sub(toBN(this.config.managerMinBalance))
        )
      ) {
        const transactionHash = await this._replenishWorker(
          workerReplenishAmount,
          currentBlockNumber,
          currentBlockHash,
          currentBlockTimestamp
        );
  ...
  async intervalHandler(): Promise<void> {
    try {
      ...
      if (block.number > this.lastScannedBlock) {
        await this._worker(block).then((transactions) => {
          if (transactions.length !== 0) {
            this.logger.debug(
              `Done handling block #${block.number}. Created ${transactions.length} transactions.`
            );
          }
        });
      }
  ...
  async _worker(block: BlockTransactionString): Promise<PrefixedHexString[]> {
    ...
    await this.withdrawToOwnerIfNeeded(
      block.number,
      block.hash,
      currentBlockTimestamp
    );
  ...
  async _refreshAndCheckBalances(): Promise<boolean> {
    ...
    if (this.shouldRefreshBalances) {
      ...
      isManagerBalanceReady =
        this.registrationManager.balanceRequired.currentValue.gte(
          toBN(this.config.managerMinBalance.toString()).divn(
            minBalanceToNotReadyFactor
          )
        );
      isWorkerBalanceReady = this.workerBalanceRequired.currentValue.gte(
        toBN(this.config.workerMinBalance.toString()).divn(
          minBalanceToNotReadyFactor
        )
      );
      ...
      const shouldReplenishManager =
        this.registrationManager.balanceRequired.currentValue.lt(
          toBN(this.config.managerMinBalance.toString())
        );
      const shouldReplenishWorker = this.workerBalanceRequired.currentValue.lt(
        toBN(this.config.workerMinBalance.toString())
      );
      this.shouldRefreshBalances =
        shouldReplenishManager || shouldReplenishWorker;
    }
    return isManagerBalanceReady;
  }

  async _refreshGasFees(): Promise<void> {
    const { baseFeePerGas, priorityFeePerGas } =
      await this.contractInteractor.getGasFees(
    ...
    this.minMaxPriorityFeePerGas = Math.floor(
      parseInt(priorityFeePerGas) * this.config.gasPriceFactor
    );
    if (
      this.minMaxPriorityFeePerGas === 0 &&
      parseInt(this.config.defaultPriorityFee) > 0
    ) {
      this.logger.debug(
    ...
    if (this.minMaxPriorityFeePerGas > parseInt(this.config.maxMaxFeePerGas)) {
      throw new Error(
        `network maxPriorityFeePerGas ${this.minMaxPriorityFeePerGas} is higher than config.maxMaxFeePerGas ${this.config.maxMaxFeePerGas}`
      );
    }

    if (this.minMaxFeePerGas > parseInt(this.config.maxMaxFeePerGas)) {
      throw new Error(
        `network minMaxFeePerGas ${this.minMaxFeePerGas} is higher than config.maxMaxFeePerGas ${this.config.maxMaxFeePerGas}`
      );
    }

    const currentNetworkFeePerGas =
      parseInt(baseFeePerGas) + parseInt(priorityFeePerGas);
    const shareOfMaximum =
      currentNetworkFeePerGas / parseInt(this.config.maxMaxFeePerGas);
    if (shareOfMaximum > 0.7) {
      this.logger.warn(
        `WARNING! Current network's reasonable fee per gas ${currentNetworkFeePerGas} is dangerously close to the config.maxMaxFeePerGas ${this.config.maxMaxFeePerGas}`
      );
    }
  }
  ...
  async _handleChanges(
    ...
    await this.transactionManager.fillMinedBlockDetailsForTransactions(
      currentBlock
    );
  ...
  async _shouldRegisterAgain(
    ...
    const relayInfo = await this.contractInteractor
      .getRelayInfo(this.managerAddress)
      .catch((e: Error) => {
    ...
    const latestRegisterTxBlockTimestamp = toNumber(
      relayInfo.lastSeenTimestamp
    );
  ...
  _shouldRefreshState(currentBlock: BlockTransactionString): boolean {
    return (
      currentBlock.number - this.lastRefreshBlock >=
    ...
    for (const event of hubEventsSinceLastScan) {
      switch (event.event) {
        case TransactionRejectedByPaymaster:
          this.logger.debug(
            `handle TransactionRejectedByPaymaster event: ${JSON.stringify(event)}`
          );
  ...
  async _boostStuckPendingTransactions(
    currentBlockInfo: ShortBlockInfo
    ...
    const managerBoostedTransactions =
      await this._boostStuckTransactionsForManager(currentBlockInfo);
  ...
  _isBlacklistedRecipient(recipient: string): boolean {
    return this.config.blacklistedRecipients
      .map((it) => it.toLowerCase())
  ...
  _isWhitelistedPaymaster(paymaster: string): boolean {
    return (
      this.config.whitelistedPaymasters.length === 0 ||
      this.config.whitelistedPaymasters
        .map((it) => it.toLowerCase())
        .includes(paymaster.toLowerCase())
    );
  }
  ...
  _isWhitelistedRecipient(recipient: string): boolean {
    return (
      this.config.whitelistedRecipients.length === 0 ||
      this.config.whitelistedRecipients
        .map((it) => it.toLowerCase())
        .includes(recipient.toLowerCase())
    );
  }

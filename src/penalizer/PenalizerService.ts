...
const MINED_TRANSACTION =
  'Transaction is the one mined on the current chain and no conflicting transaction is known to this server';
const NONCE_FORWARD =
  'Transaction nonce is higher then current account nonce and no conflicting transaction is known to this server';
...
function createWeb3Transaction(
  transaction: Web3CoreTransaction,
...
export class PenalizerService {
  ...
  async init(startWorker: boolean = true): Promise<void> {
    ...
    if (startWorker) {
      ...
      this.logger.debug(
        `Started checking for ready penalization commitments every ${this.config.checkInterval}ms`
      );
    }
  ...
  async penalizeRepeatedNonce(req: AuditRequest): Promise<AuditResponse> {
    ...
    const requestTx = EthereumJsTransaction.fromSerializedTx(
      toBuffer(req.signedTx),
      rawTxOptions
    );
    ...
    const minedTransactionData =
      await this.txByNonceService.getTransactionByNonce(
        relayWorker,
        transactionNonce
      );
    if (minedTransactionData == null) {
      throw Error(
        `TxByNonce service failed to fetch tx with nonce ${transactionNonce} of relayer ${relayWorker}`
      );
    }
    const minedTx = await this.contractInteractor.getTransaction(
      minedTransactionData.hash
    );
    if (minedTx == null) {
      throw Error(
        `Failed to get transaction ${minedTransactionData.hash} from node`
      );
    }
    const minedTxBuffers = createWeb3Transaction(minedTx, rawTxOptions);
    const randomValue = bufferToHex(crypto.randomBytes(32));
    const penalizationArguments = this.getPenalizeRepeatedNonceArguments(
      minedTxBuffers,
      requestTx,
      randomValue
    );
    const method = this.getMethod(
      PenalizationTypes.REPEATED_NONCE,
      penalizationArguments
    );
    ...
    const commitTxHash =
      await this.commitAndScheduleReveal(delayedPenalization);
    return { commitTxHash };
  }
  ...
  async executeReadyPenalizations(): Promise<PrefixedHexString[]> {
    ...
    const readyPenalizations = this.scheduledPenalizations.filter((it) => {
      return (
        it.readyBlockNumber != null && it.readyBlockNumber <= currentBlockNumber
      );
    });
    ...
    for (const penalization of readyPenalizations) {
      ...
      const penalizationTxHash =
        await this.executeDelayedPenalization(penalization);
  ...
  async queryReadyBlocksForMinedCommitments(): Promise<void> {
    const unconfirmedPenalizations = this.scheduledPenalizations.filter(
      (it) => it.readyBlockNumber === undefined
    );
    const nonMinedCommitHashes = unconfirmedPenalizations.map(
      (up) => up.commitHash
    );
    if (unconfirmedPenalizations.length > 0) {
      ...
      const commitments =
        await this.contractInteractor.getPastEventsForPenalizer(
          [CommitAdded],
          topics,
          { fromBlock: 1 }
        );
      const newlyMinedCommitments = commitments.filter((it) => {
        return nonMinedCommitHashes.includes(it.returnValues.commitHash);
      });
      unconfirmedPenalizations.forEach((it) => {
        const commitment = newlyMinedCommitments.find(
          (nmc) => nmc.returnValues.commitHash === it.commitHash
        );
        if (commitment != null) {
  ...
  async penalizeIllegalTransaction(req: AuditRequest): Promise<AuditResponse> {
    ...
    const randomValue = bufferToHex(crypto.randomBytes(32));
    const penalizationArguments = this.getPenalizeIllegalTransactionArguments(
      requestTx,
      randomValue
    );
    const method = this.getMethod(
      PenalizationTypes.ILLEGAL_TRANSACTION,
      penalizationArguments
    );
    const isValidPenalization = await this.validatePenalization(method);
  ...
  async commitAndScheduleReveal(
    delayedPenalization: DelayedPenalization
  ): Promise<any> {
    this.scheduledPenalizations.push(delayedPenalization);
    const method =
      this.contractInteractor.penalizerInstance.contract.methods.commit(
        delayedPenalization.commitHash
      );
    return await this.broadcastTransaction('commit', method);
  }

  async executeDelayedPenalization(
    delayedPenalization: DelayedPenalization
  ): Promise<PrefixedHexString> {
    const method = this.getMethod(
      delayedPenalization.type,
      delayedPenalization.methodArgs
    );
    return await this.broadcastTransaction(
      delayedPenalization.type.valueOf(),
      method
    );
  }
  ...
  async broadcastTransaction(
    ...
    this.logger.debug(
      `penalization raw tx: ${signedTx} txHash: ${transactionHash}`
    );
    return transactionHash;
  }

  getPenalizeIllegalTransactionArguments(
    requestTx: EthereumJsTransaction,
    randomValue: string
  ): PrefixedHexString[] {
    ...
    return [
      data,
      signature,
      this.contractInteractor.relayHubInstance.address,
      randomValue,
    ];
  }

  getPenalizeRepeatedNonceArguments(
    minedTx: EthereumJsTransaction,
    requestTx: EthereumJsTransaction,
    randomValue: string
  ): PrefixedHexString[] {
    const chainId = this.contractInteractor.chainId;
    const { data: unsignedMinedTx, signature: minedTxSig } =
      getDataAndSignature(minedTx, chainId);
    const { data: unsignedRequestTx, signature: requestTxSig } =
      getDataAndSignature(requestTx, chainId);
    return [
      unsignedRequestTx,
      requestTxSig,
      unsignedMinedTx,
  ...
  async validateTransaction(
    ...
    const relayWorker = bufferToHex(requestTx.getSenderAddress().toBuffer());
    const relayManager =
      await this.contractInteractor.relayHubInstance.getWorkerManager(
        relayWorker
      );
    ...
    try {
      await this.contractInteractor.relayHubInstance.verifyRelayManagerStaked(
        relayManager
      );
    } catch (e: any) {
  ...
  async isTransactionMined(requestTx: EthereumJsTransaction): Promise<boolean> {
    const txFromNode = await this.contractInteractor.getTransaction(
      bufferToHex(requestTx.hash())
    );
    return txFromNode != null;
  }

  async validatePenalization(
    method: any
  ): Promise<{ valid: boolean; error?: string }> {
    try {
      ...
      this.logger.debug(
        `view call to penalizeRepeatedNonce reverted with error message ${error}.\nTx not penalizable.`
      );
      return {
        valid: false,
        error,

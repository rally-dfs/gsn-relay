...
export async function resolveServerConfig(
  ...
  if (config.environmentName != null) {
    ...
    console.error(
      `Must provide one of the supported values for environmentName: ${Object.keys(EnvironmentsKeys).toString()}`
    );
  }
...
export function validatePrivateModeParams(config: ServerConfigParams): void {
  if (
    config.url.length !== 0 &&
    (config.whitelistedRecipients.length !== 0 ||
      config.whitelistedPaymasters.length !== 0)
  ) {
    throw new Error(
      'Cannot whitelist recipients or paymasters on a public Relay Server'
    );
  }
}

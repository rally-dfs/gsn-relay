...
function resolveReputationManagerConfiguration(
  partialConfig: Partial<ReputationManagerConfiguration>
...
export class ReputationManager {
  ...
  async getPaymasterStatus(
    ...
    try {
      const entry =
        ...
        (await this.reputationStoreManager.createEntry(
          paymaster,
          this.config.initialReputation
        ));
      ...
      if (entry.abuseStartedBlock !== 0) {
        if (
          currentBlockNumber - entry.abuseStartedBlock <=
          this.config.abuseTimeWindowBlocks
        ) {
          return PaymasterStatus.ABUSED;
        } else {
          await this.reputationStoreManager.clearAbuseFlag(
            paymaster,
            this.config.abuseTimeoutReputation
          );
        }
      }
      if (
        entry.reputation < this.config.throttleReputation &&
        Date.now() - entry.lastAcceptedRelayRequestTs <=
  ...
  async updatePaymasterStatus(
    ...
    const changeInAbuseWindow = entry.changes
      .filter(
        (it) =>
          eventBlockNumber - it.blockNumber < this.config.abuseTimeWindowBlocks
      )
      .reduce(
        (previousValue: number, currentValue: ReputationChange) =>
          previousValue + currentValue.change,
        0
      );
    if (-changeInAbuseWindow >= this.config.abuseReputationChange) {
      await this.reputationStoreManager.setAbuseFlag(
        paymaster,
        eventBlockNumber
      );
    }
    const oldChangesExpirationBlock =
      eventBlockNumber - this.config.abuseTimeWindowBlocks;
    await this.reputationStoreManager.updatePaymasterReputation(
      paymaster,
      change,
      oldChangesExpirationBlock,
      eventBlockNumber
    );
  }
}

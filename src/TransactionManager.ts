...
export class TransactionManager extends EventEmitter {
  ...
  async init(transactionType: TransactionType): Promise<void> {
    ...
    if (this.rawTxOptions == null) {
      throw new Error(
        'init failed for TransactionManager, was ContractInteractor properly initialized?'
      );
    }
  }
  ...
  printBoostedTransactionLog(
    txHash: string,
    creationBlock: ShortBlockInfo,
    maxFeePerGas: number,
    maxPriorityFeePerGas: number,
    isMaxGasPriceReached: boolean
  ): void {
    const maxFeePerGasHumanReadableOld: string = new EthVal(maxFeePerGas)
      .toGwei()
      .toFixed(4);
    const maxPriorityFeePerGasHumanReadableOld: string = new EthVal(
...
`);
  ...
  printSendTransactionLog(transaction: StoredTransaction, from: Address): void {
    ...
    const valueHumanReadable: string = new EthVal(valueString)
      .toEth()
      .toFixed(4);
    const maxFeePerGasHumanReadable: string = new EthVal(maxFeePerGas)
      .toGwei()
      .toFixed(4);
    const maxPriorityFeePerGasHumanReadable: string = new EthVal(
      maxPriorityFeePerGas
    )
      .toGwei()
      .toFixed(4);
...
`);
  ...
  async validateBalance(
    ...
    if (txCost.gt(toBN(signerBalance))) {
      throw new Error(
        `signer ${signer} balance ${signerBalance} too low: tx cost is ${txCost.toString()}`
      );
    }
  }
  ...
  async broadcastTransaction(
    ...
    try {
      ...
      if (transactionHash.toLowerCase() !== verifiedTxId.toLowerCase()) {
        throw new Error(
          `txhash mismatch: from receipt: ${transactionHash} from txstore:${verifiedTxId}`
        );
      }
  ...
  async getNonceGapFilled(
    signer: Address,
    fromNonce: number,
    toNonce: number
  ): Promise<ObjectMap<PrefixedHexString>> {
    const nonceGap: ObjectMap<PrefixedHexString> = {};
    const transactions = await this.txStoreManager.getTxsInNonceRange(
      signer,
      fromNonce,
      toNonce
    );
  ...
  async sendTransaction(
    txDetails: SendTransactionDetails
  ): Promise<SignedTransactionDetails> {
    const encodedCall = txDetails.method?.encodeABI() ?? '0x';
    const maxFeePerGas = parseInt(
      txDetails.maxFeePerGas ?? (await this.gasPriceFetcher.getGasPrice())
    );
    const maxPriorityFeePerGas = parseInt(
      txDetails.maxPriorityFeePerGas ?? maxFeePerGas.toString()
    );
    ...
    try {
      ...
      const keyManager = this.managerKeyManager.isSigner(txDetails.signer)
        ? this.managerKeyManager
        : this.workersKeyManager;
      signedTransaction = keyManager.signTransaction(
        txDetails.signer,
        txToSign
      );
      ...
      storedTx = createStoredTransaction(
        signedTransaction.signedEthJsTx,
        metadata
      );
  ...
  async resendTransaction(
    ...
    if (
      tx.maxFeePerGas < storedTx.maxFeePerGas ||
      tx.maxPriorityFeePerGas < storedTx.maxPriorityFeePerGas
    ) {
    ...
    this.logger.debug(
      `Current account nonce for ${tx.from} is ${currentNonce}`
    );
  ...
  _resolveNewGasPrice(
    ...
    let newMaxPriorityFee = Math.round(
      oldMaxPriorityFee * this.config.retryGasPriceFactor
    );
    if (newMaxPriorityFee < minMaxPriorityFee) {
      this.logger.warn(
        `Adjusting newMaxPriorityFee ${newMaxPriorityFee} to current minimum of ${minMaxPriorityFee}`
      );
      newMaxPriorityFee = minMaxPriorityFee;
    }
    if (newMaxFee < minMaxFee) {
      this.logger.warn(
        `Adjusting minMaxFee ${newMaxFee} to current minimum of ${minMaxFee}`
      );
      newMaxFee = minMaxFee;
    }
    ...
    if (newMaxFee > parseInt(this.config.maxMaxFeePerGas)) {
      ...
      this.logger.warn(
        `Adjusting newMaxFee ${newMaxFee} to maxFeePerGas ${this.config.maxMaxFeePerGas}`
      );
      newMaxFee = parseInt(this.config.maxMaxFeePerGas);
    }
    if (newMaxPriorityFee > newMaxFee) {
      this.logger.warn(
        `Adjusting newMaxPriorityFee ${newMaxPriorityFee} to newMaxFee ${newMaxFee}`
      );
  ...
  async pollNonce(signer: Address): Promise<number> {
    ...
    if (nonce > this.nonces[signer]) {
      this.logger.warn(
        `NONCE FIX for signer: ${signer} | new nonce: ${nonce} | wrong nonce: ${this.nonces[signer]}`
      );
  ...
  async removeArchivedTransactions(
    currentBlock: ShortBlockInfo
  ): Promise<unknown> {
    const upToBlockNumber =
      currentBlock.number - this.config.dbPruneTxAfterBlocks;
    const upToTimestamp =
      parseInt(currentBlock.timestamp.toString()) -
      this.config.dbPruneTxAfterSeconds;
    this.logger.debug(
      `removing stored transactions that are older than block number #${upToBlockNumber} and sent before ${upToTimestamp}`
    );
    return await this.txStoreManager.removeArchivedTransactions(
      upToBlockNumber,
      upToTimestamp
    );
  }

  async fillMinedBlockDetailsForTransactions(
    currentBlock: ShortBlockInfo
  ): Promise<void> {
    ...
    this.logger.debug(
      `Total of ${sortedTxs.length} transactions are currently in the database, checking...`
    );
    for (const transaction of sortedTxs) {
      const shouldRecheck =
        transaction.minedBlock?.number == null || // transaction has never returned a receipt
        currentBlock.number - transaction.minedBlock.number >=
          this.config.dbPruneTxAfterBlocks; // transaction was mined recently - rudimentary check for uncling or reorg
      if (shouldRecheck) {
        ...
        if (receipt.blockNumber !== transaction.minedBlock?.number) {
          ...
          this.logger.debug(
            `Tx ${transaction.txId} was mined and has ${confirmations} confirmations`
          );
          const minedBlock = await this.contractInteractor.getBlock(
            receipt.blockNumber
          );
  ...
  async boostUnderpricedPendingTransactionsForSigner(
    ...
    const pendingTxs = await this.txStoreManager.getTxsInNonceRange(
      signer,
      nonce
    );
    ...
    if (nonce !== oldestPendingTx.nonce) {
      throw new Error(
        `Boosting: missing nonce ${nonce}. Lowest stored tx nonce: ${oldestPendingTx.nonce}`
      );
    }

    const lastSentAtBlockTimestamp = parseInt(
      (
        oldestPendingTx.boostBlock?.timestamp ??
        oldestPendingTx.creationBlock.timestamp
      ).toString()
    );
    // If the tx is still pending, check how long ago we sent it, and resend it if needed
    if (
      parseInt(currentBlock.timestamp.toString()) - lastSentAtBlockTimestamp <
      this.config.pendingTransactionTimeoutSeconds
    ) {
      this.logger.debug(
        `${signer} : awaiting transaction with ID: ${oldestPendingTx.txId} to be mined. creationBlockNumber: ${oldestPendingTx.creationBlock.number} creationBlockHash: ${oldestPendingTx.creationBlock.hash} nonce: ${nonce}`
      );
      return boostedTransactions;
    }
    ...
    const { newMaxFee, newMaxPriorityFee, isMaxGasPriceReached } =
      this._resolveNewGasPrice(
        oldestPendingTx.maxFeePerGas,
        oldestPendingTx.maxPriorityFeePerGas,
        minMaxPriorityFee,
        parseInt(gasFees.baseFeePerGas)
      );
    for (const transaction of pendingTxs) {
      // The tx is underpriced, boost it
      if (
        transaction.maxFeePerGas < newMaxFee ||
        transaction.maxPriorityFeePerGas < newMaxPriorityFee
      ) {
        ...
        // The tx is ok, just rebroadcast it
        try {
          await this.resendTransaction(
            transaction,
            currentBlock,
            transaction.maxFeePerGas,
            transaction.maxPriorityFeePerGas,
            transaction.maxFeePerGas > parseInt(this.config.maxMaxFeePerGas)
          );
        } catch (e: any) {
          this.logger.error(
            `Rebroadcasting existing transaction: ${(e as Error).message}`
          );
        }
      }
